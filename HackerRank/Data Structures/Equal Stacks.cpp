...
int equalStacks(vector<int> h1, vector<int> h2, vector<int> h3) {
    ...
    int s = 0;
    m1.insert(s);
    for(auto rit = h1.rbegin(); rit!= h1.rend(); ++rit){
        s+= *rit;
        m1.insert(s);
    }
    ...
    s = 0;
    m2.insert(s);
    for(auto rit = h2.rbegin(); rit!= h2.rend(); ++rit){
        s+= *rit;
        m2.insert(s);
    }
    ...
    s = 0;
    m3.insert(s);
    for(auto rit = h3.rbegin(); rit!= h3.rend(); ++rit){
        s+= *rit;
        m3.insert(s);
    }
    ...
    for(auto it = m1.begin(); it!= m1.end(); ++it ){
        if(m2.find(*it)!=m2.end() && m3.find(*it)!=m3.end()){
            if(height < *it)
                height = *it;
        }
    }
